...
void SIM_Send_Int(unsigned long n){
   ...
   unsigned char i,j;
	 ...
	 for(j = i+1; j <= 15; j++) {
    	HAL_UART_Transmit(&huart1,(uint8_t *)&buffer[j], 1, 1000);
     }
}
/**
...
char* SIM_replyCommand(int _timeout){
	...
	do{ //strstr(UARTSim.Rx_temp, "\r\nOK\r\n") ||
		if(strstr(UARTSim.Rx_temp, "\r\nOK\r\n") ||strstr(UARTSim.Rx_temp, "\r\nERROR\r\n")||strstr(UARTSim.Rx_temp, "not allowed\r\n")){
			break;
		}
	}
...
void SIM_sendSMS(char* targetNumber, char* message){
	...
	HAL_Delay(300);
	HAL_UART_Transmit(&huart1,(uint8_t *)&dataSMS.sendSMS, 1, 1000);
	SIM_replyCommand(5000);
	SIM_Delete_Reply();
